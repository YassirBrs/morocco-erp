...
import { getEnterpriseIntelligenceReadiness, type EnterpriseIntelligenceReadiness } from '../../lib/api';
import {
  enterpriseIntelligenceFeatureByKey,
  enterpriseIntelligenceFeatureDefinitions,
  type EnterpriseIntelligenceFeatureKey,
  type EnterpriseIntelligenceMetric,
} from './enterprise-intelligence-feature-config';

type EnterpriseIntelligenceFeaturePageProps = {
  data: EnterpriseIntelligenceReadiness;
  featureKey: EnterpriseIntelligenceFeatureKey;
};

export function makeEnterpriseIntelligenceServerSideProps(featureKey: EnterpriseIntelligenceFeatureKey): GetServerSideProps<EnterpriseIntelligenceFeaturePageProps> {
  return async () => ({
    props: {
      data: await getEnterpriseIntelligenceReadiness(),
      featureKey,
    },
  });
}

export function EnterpriseIntelligenceFeaturePage({ data, featureKey }: EnterpriseIntelligenceFeaturePageProps) {
  const definition = enterpriseIntelligenceFeatureByKey[featureKey];
      ...
      <main className="shell featureShell">
        <aside className="sidebar">
          <div className="brand">
            ...
            <div>
              ...
              <small>Pages intelligence Maroc</small>
            </div>
          </div>
          <nav aria-label="Fonctionnalités intelligence entreprise">
        ...
        <section className="workspace">
          ...
          <section className="gridTwo">
            <div className="panel">
              <PanelHeader title="Contrôles métier" action="Ouvrir analyse" />
              <ul className="checks">
                {definition.controls.map((control) => <li key={control}>{control}</li>)}
              </ul>
            </div>
            <div className="panel">
              <PanelHeader title="Données suivies" action="Exporter JSON" />
              <DataTable
...
function formatMetric(source: unknown, metric: EnterpriseIntelligenceMetric): string {
