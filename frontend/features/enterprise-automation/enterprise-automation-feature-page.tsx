...
import { getEnterpriseAutomationReadiness, type EnterpriseAutomationReadiness } from '../../lib/api';
import {
  enterpriseAutomationFeatureByKey,
  enterpriseAutomationFeatureDefinitions,
  type EnterpriseAutomationFeatureKey,
  type EnterpriseAutomationMetric,
} from './enterprise-automation-feature-config';

type EnterpriseAutomationFeaturePageProps = {
  data: EnterpriseAutomationReadiness;
  featureKey: EnterpriseAutomationFeatureKey;
};

export function makeEnterpriseAutomationServerSideProps(featureKey: EnterpriseAutomationFeatureKey): GetServerSideProps<EnterpriseAutomationFeaturePageProps> {
  return async () => ({
    props: {
      data: await getEnterpriseAutomationReadiness(),
      featureKey,
    },
  });
}

export function EnterpriseAutomationFeaturePage({ data, featureKey }: EnterpriseAutomationFeaturePageProps) {
  const definition = enterpriseAutomationFeatureByKey[featureKey];
      ...
      <main className="shell featureShell">
        <aside className="sidebar">
          <div className="brand">
            ...
            <div>
              ...
              <small>Pages automatisation Maroc</small>
            </div>
          </div>
          <nav aria-label="Fonctionnalités automatisation entreprise">
        ...
        <section className="workspace">
          ...
          <section className="gridTwo">
            <div className="panel">
              <PanelHeader title="Contrôles métier" action="Ouvrir contrôle" />
              <ul className="checks">
...
function formatMetric(source: unknown, metric: EnterpriseAutomationMetric): string {
