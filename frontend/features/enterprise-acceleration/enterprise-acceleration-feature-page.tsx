...
import { getEnterpriseAccelerationReadiness, type EnterpriseAccelerationReadiness } from '../../lib/api';
import {
  enterpriseAccelerationFeatureByKey,
  enterpriseAccelerationFeatureDefinitions,
  type EnterpriseAccelerationFeatureKey,
  type EnterpriseAccelerationMetric,
} from './enterprise-acceleration-feature-config';

type EnterpriseAccelerationFeaturePageProps = {
  data: EnterpriseAccelerationReadiness;
  featureKey: EnterpriseAccelerationFeatureKey;
};

export function makeEnterpriseAccelerationServerSideProps(featureKey: EnterpriseAccelerationFeatureKey): GetServerSideProps<EnterpriseAccelerationFeaturePageProps> {
  return async () => ({
    props: {
      data: await getEnterpriseAccelerationReadiness(),
      featureKey,
    },
  });
}

export function EnterpriseAccelerationFeaturePage({ data, featureKey }: EnterpriseAccelerationFeaturePageProps) {
  const definition = enterpriseAccelerationFeatureByKey[featureKey];
        ...
        <aside className="sidebar">
          <div className="brand">
            ...
            <div>
              ...
              <small>Pages accélération Maroc</small>
            </div>
          </div>
          <nav aria-label="Fonctionnalités accélération entreprise">
        ...
        <section className="workspace">
          <header className="topbar">
            ...
            <div className="status">
              ...
              <strong>Accélération active</strong>
            </div>
          </header>
...
function formatMetric(source: unknown, metric: EnterpriseAccelerationMetric): string {
